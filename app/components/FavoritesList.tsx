    ...
    <div className="space-y-6">
      ...
      {activeMenu && (
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Rechercher dans vos favoris..."
            className="w-full pl-10 pr-4 py-2.5 bg-gray-800/50 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
          />
        </div>
        ...
        <div className="flex items-center gap-3">
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as typeof sortBy)}
            className="bg-gray-800/50 border border-gray-700 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="recent">Récemment ajoutés</option>
            <option value="title">Nom (A-Z)</option>
            <option value="status">Statut de lecture</option>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {filteredAndSortedFavorites.map((manga) => (
          <div
            key={manga.id}
            className="group relative bg-gray-800/30 rounded-lg overflow-hidden border border-gray-800 hover:border-gray-700 transition-all duration-300 hover:transform hover:-translate-y-1"
          >
            {/* Image de couverture */}
            <div className="relative aspect-[3/4] cursor-pointer" onClick={() => handleMangaClick(manga.id)}>
              <Image
              ...
              <div className="absolute top-2 left-2">
                {getStatusBadge(manga.readingStatus)}
              </div>
              ...
              <div className="absolute top-2 right-2">
                <button
                  ...
                  className="p-1.5 rounded-full bg-black/50 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:bg-black/70"
                >
                  <MoreHorizontal className="w-4 h-4 text-white" />
                </button>
                ...
                {/* Menu dropdown */}
                {activeMenu === manga.id && (
                  <div className="absolute top-8 right-0 w-48 bg-gray-900 border border-gray-700 rounded-lg shadow-xl z-10 overflow-hidden">
                    <div className="py-1">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2 text-sm bg-transparent text-gray-300 hover:bg-gray-800 border-0 focus:outline-none"
                      >
                        <option value="to-read">À lire</option>
                        <option value="reading">En cours</option>
                        <option value="completed">Terminé</option>
              ...
              {manga.chapterCount?.total && (
                <div className="absolute bottom-2 right-2">
                  <span className="px-2 py-1 text-xs font-medium bg-black/70 backdrop-blur-sm text-white rounded-full">
            ...
            <div className="p-3">
              <h3 
                className="font-medium text-white text-sm mb-1 line-clamp-2 cursor-pointer hover:text-blue-400 transition-colors"
                onClick={() => handleMangaClick(manga.id)}
              >
                {manga.title}
              </h3>
            ...
            {/* Modal d'édition de note */}
            {editingNote === manga.id && (
              <div className="absolute inset-0 bg-gray-900/95 backdrop-blur-sm p-4 flex flex-col justify-center z-20">
                <textarea
                  value={noteText}
                  onChange={(e) => setNoteText(e.target.value)}
                  placeholder="Votre note personnelle..."
                  className="w-full p-2 text-sm bg-gray-800 border border-gray-600 rounded text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                  rows={3}
                  autoFocus
                />
